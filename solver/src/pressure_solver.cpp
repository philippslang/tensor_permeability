...
namespace csmp {
	namespace tperm{
		...
		*/
		...
		void primary_bc(const Boundaries::OpposingBoundaries& bdp, const Index& k, double half_dp)
		{
		...
		{
			auto AB = pts[1]-pts[0];
			auto AC = VectorVariable<3>(pts[2].Coordinates()) - VectorVariable<3>(pts[0].Coordinates());
		...
		void supplementary_bc(const Boundaries& bds, const Index& k, double half_dp, size_t d)
		{
			...
			const auto b2b_dist = tpl_point2plane_dist(bin_plane, bds[d].second[0]->Coordinate());
			...
			for (size_t i(0); i < bds.size(); ++i) {
				...
				for (const auto& b : { bds[i].first, bds[i].second })
					for (size_t n(0); n < b.size(); ++n) {
						const double lp = half_dp-tpl_point2plane_dist(bin_plane, b[n]->Coordinate())*gradp;
		...
		*/
		void pgrad_and_vel(csmp::Model<3>& m, size_t d)
		{
			...
			const Index pgKey(m.Database().StorageKey(rvnames[0].c_str())), vKey(m.Database().StorageKey(rvnames[1].c_str()));			
			const Index kKey(m.Database().StorageKey("permeability")), pKey(m.Database().StorageKey("fluid pressure"));
