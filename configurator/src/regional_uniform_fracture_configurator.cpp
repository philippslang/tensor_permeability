...
namespace csmp {
	namespace tperm {
		...
		/**
		This ctor causes subsequently that all fracture elements of the same region hold the same hydraulic aperture tensor, thus need to have the same
		orientation in space. The tensor needs to reflect that the fracture normal direction aperture is zero, and that the eigenvalues
		are aligned in the fracture plane, plus the unormal. Because this might be a rotated tensor, we need aperture, permeability (ah^2/12, then rotated)
		and conductivity (ah^3/12, then rotated) in full tensor form for each region.
		*/
		RegionalUniformFractureConfigurator::RegionalUniformFractureConfigurator(const vector<csmp::TensorVariable<3>>& ahs,
																				 ...
																				 const vector<csmp::TensorVariable<3>>& cs,
																				 const vector<double>& ams,
																				 const vector<string>& rnames)
			: UniformFractureConfigurator(csmp::TensorVariable<3>(PLAIN, 0.), csmp::TensorVariable<3>(PLAIN, 0.), csmp::TensorVariable<3>(PLAIN, 0.), 0.), 
		...
		/**
		Attention: Expensive version. Subsequently this causes for all fracture elements in one region to have their unit normal
		...
		*/
		RegionalUniformFractureConfigurator::RegionalUniformFractureConfigurator(const vector<double>& ahs,
																				 const vector<double>& ams,
																				 const vector<string>& rnames)
			: UniformFractureConfigurator(0., 0.), ams_(ams), frnames_(rnames), ahs_(ahs.size(), this->ah_), 
			  ks_(ahs.size(), this->ah_), cs_(ahs.size(), this->ah_)
		{
		...
		/**
		Assigns uniform transmissivity for each provided region. Depending on the construction, this
