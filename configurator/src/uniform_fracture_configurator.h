...
protected:
  ...
  void project_ah(Model &, const std::vector<Element<3> *> &,
                  const csmp::TensorVariable<3> &) const;
  void direct_ah(Model &m, const std::vector<Element<3> *> &,
                 const csmp::TensorVariable<3> &ah,
                 const csmp::TensorVariable<3> &k,
                 const csmp::TensorVariable<3> &c) const;

  const bool project_; ///< Hydraulic aperture is isotropic (only ah_xx and
                       ///ah_yy set) and needs projection
  const double am_;
