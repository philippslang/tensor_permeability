...
UniformMatrixConfigurator::UniformMatrixConfigurator(
    const csmp::TensorVariable<3> &perm)
    : Configurator(), perm_(perm) {}

UniformMatrixConfigurator::UniformMatrixConfigurator(double perm)
    : Configurator(), perm_(csmp::TensorVariable<3>(PLAIN, perm, 0., 0., 0.,
                                                    perm, 0., 0., 0., perm)) {}

/**
Assigns permeability and conductivity to all equi-dimensional
elements in `Model`. Relies on the following variables:
