...
/**
This ctor causes subsequently that all fracture elements hold the
same hydraulic aperture tensor, thus need to have the same
orientation in space. The tensor needs to reflect that the fractre
normal direction aperture is zero, and that the eigenvalues
are aligned in the fracture plane, plus the unormal.
*/
UniformFractureConfigurator::UniformFractureConfigurator(
    const csmp::TensorVariable<3> &ah, const csmp::TensorVariable<3> &k,
    const csmp::TensorVariable<3> &c, double am)
    : Configurator(), project_(false), am_(am), ah_(ah), k_(k), c_(c) {}

/**
Attention: Expensive version. Subsequently this causes for all
fracture elements to have their unit normal
...
UniformFractureConfigurator::UniformFractureConfigurator(double ah, double am)
    : Configurator(),
      ah_(csmp::TensorVariable<3>(PLAIN, ah, 0., 0., 0., ah, 0., 0., 0., 0.)),
      am_(am), project_(true) {}

/**
Assigns uniform transmissivity to all lower-dimensional elements in
`Model`. Depending on the construction, this
...
void UniformFractureConfigurator::input_am(
    Model &model, const std::vector<Element<3> *> &felmts, double a) const {
...
/// Projects (x-y) isotropic hydraulic aperture, permeability and conductivity
/// tensor, expects zero components except for ah_xx and ah_yy
void UniformFractureConfigurator::direct_ah(
    Model &model, const std::vector<Element<3> *> &felmts,
    const csmp::TensorVariable<3> &ah, const csmp::TensorVariable<3> &k,
...
/// Projects (x-y) isotropic hydraulic aperture tensor, expects zero components
/// except for ah_xx and ah_yy
void UniformFractureConfigurator::project_ah(
    Model &model, const std::vector<Element<3> *> &felmts,
    const csmp::TensorVariable<3> &ah) const {
  const array<Index, 3> keys = {
      Index(model.Database().StorageKey("hydraulic aperture")),
      Index(model.Database().StorageKey("permeability")),
      Index(model.Database().StorageKey("conductivity"))};
  // corresponding to indices in keys
  array<TensorVariable<3>, 3> ts_loc = {
      TensorVariable<3>(ah), TensorVariable<3>(ah), TensorVariable<3>(ah)};
  ts_loc[1].Power(2.);
  ts_loc[1] /= 12.; // m2
  ts_loc[2].Power(3.);
  ts_loc[2] /= 12.; // m3 / Pa.s
  // working variables
  array<VectorVariable<3>, 3>
      eloc; // element plane unit vectors w.r.t. glob coord-system
