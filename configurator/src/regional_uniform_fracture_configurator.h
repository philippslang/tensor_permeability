...
public:
  ...
  RegionalUniformFractureConfigurator(
      const std::vector<csmp::TensorVariable<3>> &ahs,
      const std::vector<csmp::TensorVariable<3>> &ks,
      const std::vector<csmp::TensorVariable<3>> &cs,
      const std::vector<double> &ams, const std::vector<std::string> &rnames);
...
Configures (3D model) equidimensional elements to a uniform
`hydraulic aperture` and `mechanical aperture` and corresponding
`conductivity` (m3/Pa.s) and `permeability` (m2) Depending on the
chosen ctor, either a provided tensor will be assigned to all,
or an isotropic hydraulic aperture will be  assigned by projecting
the appropriate tensor. The latter requires and element-wise
computation of the unit normal. This is expensive.

The provided vectors for the ctors are of identical size and
correspond to each region to be
initialized. It is the users responsability to include all fratcure
regions, so that no uninitialized parts of the model
