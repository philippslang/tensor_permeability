...
namespace csmp {
	namespace tperm {
		...
		void to_screen(string rname, const UpscaledTensor& t, const vector<double>& evals)
		{
		...
		@todo Dispatch screen output, store evals in structure and reuse for later screen IO
		@todo Dispatch screen output, store evals in structure and reuse for later screen IO
		*/
		void report(const std::map<std::string, UpscaledTensor>& omega_tensors, const Model<3>& m, string fname)
		{
			announce_report();
		...
		void vtu(const std::map<std::string, UpscaledTensor>& omega_tensors, const Model<3>& m, const char* fname)
		{
