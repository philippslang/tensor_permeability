...
bool eigen_values(const UpscaledTensor &t, vector<double> &evals,
                  vector<vector<double>> &evecs) {
  typedef Eigen::Matrix<double, 3, 3> Matrix3d;
  Matrix3d eigenKeff;
  eigenKeff << t(0, 0), t(0, 1), t(0, 2), t(1, 0), t(1, 1), t(1, 2), t(2, 0),
      t(2, 1), t(2, 2);
...
vector<double> solve_overdet(const DenseMatrix<12> &A,
...
UpscaledTensor analyze(const std::vector<FlowResults> &frv,
  ...
  // averaging
  const auto &volumes =
      frv[0].volumes(); // assumption: element info for all exps identical
  const auto elements =
      volumes.size(); // assumption: element info for all exps identical
  // average xy(z) components for pressure gradients and velocities for each
  // experiment (outer index)
  // and each component (inner index)
  vector<vector<double>> vel_avgs(d, vector<double>(d)),
      pgrad_avgs(d, vector<double>(d));
  ...
  vector<double64> u(d * d + odiags,
                     0.); // rhs, velocity component averages plus zeros
