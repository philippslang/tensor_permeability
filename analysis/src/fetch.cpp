...
/**
@todo Need an indirection here for omega collection, mediator class
to loosen coupling with OmegaGenerator
*/
map<string, UpscaledTensor>
fetch(const csmp::Model<3> &m,
      const std::map<std::string, std::shared_ptr<Omega>> &ocoll) {
...
@todo We have kind of a redundncy here in that FlowResults contain
all element volumes while they're contained in Omega
*/
FlowResults fetch(size_t d, const Omega &o, const csmp::Model<3> &m) {
  ...
  const Index v_key(
      m.Database().StorageKey(((string) "velocity " + to_string(d)).c_str()));
  const Index pg_key(m.Database().StorageKey(
      ((string) "pressure gradient " + to_string(d)).c_str()));

  vector<double> evols(elements);
  vector<vector<double>> vel_comps(md, vector<double>(elements)),
      pg_comps(md, vector<double>(elements));
  ...
  for (size_t id(0); id < md; ++id) {
    fr.assign_components(FlowResults::Data::velocity, id, vel_comps[id].begin(),
                         vel_comps[id].end());
    fr.assign_components(FlowResults::Data::pressure_gradient, id,
                         pg_comps[id].begin(), pg_comps[id].end());
  }
  return fr;
}

void make_omega_regions(
    const std::map<std::string, std::shared_ptr<Omega>> &omegas,
    csmp::Model<3> &m) {
  m.UpdateIndices();
  for (const auto &o : omegas) {
    size_t i(0);
    vector<size_t> eids(o.second->size(), 0);
    for_each(o.second->cbegin(), o.second->cend(),
             [&](const auto &einf) { eids[i++] = einf.ePtr->Idx(); });
    m.FormRegionFrom(o.first.c_str(), eids);
  }
}

std::map<std::string, std::shared_ptr<Omega>>
named_omegas(const std::vector<std::shared_ptr<Omega>> &omegas) {
