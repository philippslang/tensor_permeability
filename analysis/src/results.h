...
public:
  ...
  template <typename InputIterator>
  void assign_components(Data r, size_t component, InputIterator first,
                         InputIterator last) {
    if (r == Data::velocity)
  ...
  /**@deprecated Use omega volume*/
  virtual double domain_volume() const {
    return accumulate(vols_.begin(), vols_.end(), 0.);
  }

private:
  std::vector<double> vols_; ///< Element volumes
  std::vector<std::vector<double>>
      pgrad_comps_;                            ///< Pressure gradient components
...
public:
  ...
  double &operator()(size_t r, size_t c) {
    return t_.at(row_column_to_idx(r, c));
  }
