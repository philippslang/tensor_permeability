...
/**
Computes element volume. For fracture elements, this requires the
variable
...
\endcode
*/
Omega omega_from_elements(const vector<csmp::Element<3> *> &elmts,
                          const csmp::Index &am_key) {
  Omega o;
  o.resize(elmts.size());
...
double
OmegaGenerator::total_volume(const std::array<csmp::Point<3>, 2> &cmm) const {
  double tv(1.0);
