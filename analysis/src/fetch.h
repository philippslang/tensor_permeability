...
std::map<std::string, UpscaledTensor>
fetch(const csmp::Model<3> &,
      const std::map<std::string, std::shared_ptr<Omega>> &);
...
named_omegas(const std::vector<std::shared_ptr<Omega>> &);

/// Creates a region for each Omega with corresponding name
void make_omega_regions(const std::map<std::string, std::shared_ptr<Omega>> &,
                        csmp::Model<3> &);
