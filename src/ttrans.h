...
namespace csmp {
	namespace tperm {
		...
		/**
		The plane represented by the three points `pts` is assumed the fracture plane. The third local `e3loc`
		...
		*/
		inline void local_uvecs(const std::array<csmp::Point<3>, 3>& pts, std::array<csmp::VectorVariable<3>, 3>& eloc)
		{
			...
			eloc[2].EuclideanNormalize(); // e3loc is unit normal to element(fracture) plane, and local z coord
			eloc[1](0) = 0.; eloc[1](1) = 1.; eloc[1](2) = 0.; // global e2
		...
		inline void trans_tensor(const std::array<csmp::VectorVariable<3>, 3>& eloc, VectorVariable<3>& dcosine, TensorVariable<3>& ttrans)
		{
