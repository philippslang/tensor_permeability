...
public:
  explicit json_null(const char *missing_object)
      : miss_obj_name_(missing_object) {}
  virtual const char *what() const throw() {
    return std::string("JSON object " + (std::string)miss_obj_name_ +
                       " doesn't exist")
        .c_str();
  }
};
...
The class currently subclasses nlohmann::json. It is the interface
